...
one_hour_m6a <- new_ratio_bayesian_p_de %>% 
    ...
    filter(!is.na(symbol)) %>% 
    unique() %>% 
    mutate(m6a_site = symbol %in% m6a$symbol) %>% 
    filter(time == 1) %>% 
    mutate(total_new = paste(total_rna_sig,new_rna_sig,sep = "_")) %>% 
...
corrplot(one_hour_m6a$residuals, is.cor = FALSE, title = "one hour m6a correlation")
corrplot(two_hour_m6a$residuals, is.cor = FALSE, title = "two hour m6a correlation")
corrplot(six_hour_m6a$residuals, is.cor = FALSE, title = "6 hour m6a correlation")

corrplot(100*one_hour_m6a$residuals^2/one_hour_m6a$statistic,is.corr = FALSE, title = "one hour m6a correlation - contribution per cell")
corrplot(100*two_hour_m6a$residuals^2/two_hour_m6a$statistic,is.corr = FALSE, title = "two hour m6a correlation - contribution by cell")
corrplot(100*six_hour_m6a$residuals^2/six_hour_m6a$statistic,is.corr = FALSE, title = "six hour m6a correlation - contribution by cell")
```
...
wanna see what bdnf does to m6a writer complex: METTL3, METTL14, WTAP; and m6a eraser: ALKBH5
```{r bdnf on m6a writers}
writer_m6a <- new_ratio_bayesian_p_de %>% 
  filter(gene_name == c("METTL3", "METTL14", "WTAP"))
```
